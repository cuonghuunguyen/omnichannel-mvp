...
async function main() {
  // The human operator that escalations (disputes, refunds) are routed to.
  ...
  // Default entry agent: the Concierge greets the guest and routes. It is the
  // first thing every guest talks to, so it carries guardrails too — enabled
  // with a broad hotel scope so off-topic/injection is blocked at the door,
  // while still allowing any genuine hotel request through to a specialist.
  await db.agent.upsert({
    ...
    create: {
      ...
      handoffRules: JSON.stringify([
        { when: { keywords: ["refund", "dispute", "chargeback", "billing"] }, assignTo: human.id },
        { when: {}, assignTo: "queue" },
      ]),
  ...
  await db.agent.upsert({
    ...
    create: {
      ...
      isRoutable: true,
      builtinTools: JSON.stringify({ sendMessage: true, deliverToAgent: true, deliverToHuman: true, endConversation: true }),
      handoffRules: JSON.stringify([{ when: {}, assignTo: human.id }]),
      guardrails: JSON.stringify({
        enabled: true,
        scope:
  ...
  console.log("Seeded: 1 human agent, 3 AI agents (Concierge default, Reservations, Guest Services).");
}
