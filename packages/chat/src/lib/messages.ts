...
/** A message row optionally joined with its human author (for the UI badge).
 * The AI author's name is denormalized onto the row (authorAgentName) since the
 * Agent lives in another service. */
...
export function toUIMessage(m: MessageWithAuthors): ChatUIMessage {
  ...
  if (m.authorAgentId) {
    metadata = {
      ...
      agentName: m.authorAgentName ?? undefined,
      authorKind: "ai",
    };
