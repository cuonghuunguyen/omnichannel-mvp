 ...
 * (e.g. mysql://app:app@localhost:3307/chat). The Prisma 7 MySQL driver adapter
 * takes discrete connection fields rather than a connection string.
 */
export function mariaConfig() {
  const url = new URL(process.env.DATABASE_URL ?? "mysql://app:app@localhost:3307/chat");
  return {
    host: url.hostname,
