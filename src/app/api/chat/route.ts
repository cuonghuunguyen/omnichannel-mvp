...
export async function POST(req: Request) {
  ...
  const entryAgent = conversation.currentAgent;
  if (!entryAgent) {
  ...
  const { guardrails } = parseAgentConfig(entryAgent);
  ...
  if (verdict?.blocked) {
    ...
    const saved = await db.message.create({
      ...
      include: { authorAgent: { select: { name: true } } },
    });
    publish(conversationId, {
      kind: "message",
      origin: "ai",
    ...
    execute: async ({ writer }) => {
      ...
      for (let hop = 0; hop < MAX_HOPS; hop++) {
        ...
        if (signal.kind === "human") {
          ...
          const { handoffRules } = parseAgentConfig(current);
          const { humanAgentId } = evaluateHandoffRules(handoffRules, {
        ...
        const next = await db.agent.findUnique({
          where: { id: signal.targetAgentId },
        });
